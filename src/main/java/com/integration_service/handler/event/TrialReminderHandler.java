...
public class TrialReminderHandler implements EventHandler {

    private final DispatcherService dispatcherService;

    @Override
    public void handle(EventRequest request) {
        log.info("Processing TRIAL_REMINDER for tenant: {}", request.getTenantId());
        dispatcherService.dispatch(request, request.getTenantId());
    }

    @Override
    public String getEventType() {
        return EventTypes.TRIAL_REMINDER;
    }
}
