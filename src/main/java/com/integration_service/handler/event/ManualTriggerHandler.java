...
public class ManualTriggerHandler implements EventHandler {

    private final DispatcherService dispatcherService;

    @Override
    public void handle(EventRequest request) {
        log.info("Processing MANUAL_TRIGGER for tenant: {}", request.getTenantId());
        dispatcherService.dispatch(request, request.getTenantId());
    }

    @Override
    public String getEventType() {
        return EventTypes.MANUAL_TRIGGER;
    }
}
